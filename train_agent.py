...
def get_target_policies(prev_bids, players_dice, agent, policy_prob):
    ...
    for i in range(0, len(policy_prob)):
        ...
        target_policies[i][start_ind:] = policy_prob[i][start_ind:] + next_values_array - np.mean(next_values_array)
...
def get_target_values(payoff, history):
    ...
    for i in range(2, len(history)):
        target_values.append(target_values[-2] + 0)
...
if __name__ == '__main__':
    ...
    for i in range(0, max_iter):
        ...
        policy_targets = get_target_policies(prev_bids, [o_player1, o_player2], player, policy_probs)
        states = get_states(prev_bids, [o_player1, o_player2])
        ...
        value_loss = player.train_Value_net(value_states, Value_targets)
